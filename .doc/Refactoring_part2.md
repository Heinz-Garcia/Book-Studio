...
| 2 | Schritt 2.1 (WorkspaceService) | 1–2 h | gering | Methoden `_get_projects_root_path` und `_discover_projects` aus `BookStudio` in `services/workspace_service.py` verschieben. `BookStudio` ruft den Service via `self._services.workspace` auf; dünner Wrapper bleibt für Backward-Compat. **Status: erledigt (Commit `XXXXXXX`)**. |
