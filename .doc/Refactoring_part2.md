...
| 6 | Schritt 4 (CI verschärfen) | 30 min | gering | `--cov-fail-under=80` in `pytest.ini` setzen, `.pre-commit-config.yaml` (Ruff + flake8 + py_compile) installieren und in `.github/workflows/ci.yml` einbinden. CI schlägt ab jetzt fehl, wenn Coverage < 80 % fällt. **Status: erledigt (Commit folgt am Ende dieser Session)** — `pytest.ini` hat jetzt `fail_under=80`, `.flake8` und `ruff.toml` definieren konsistente Excludes für Legacy-Module, CI-Workflow hat dedizierte Lint- und Compile-Schritte vor Pytest, Pre-Commit nutzt die zentralen Configs. |
