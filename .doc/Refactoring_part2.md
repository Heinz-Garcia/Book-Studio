...
| 3 | Schritt 2.2 (BookSessionService) | 2–3 h | mittel | `load_book` plus Profile-Loading-Logik aus `BookStudio` in `services/book_session_service.py` verlagern. Aktives Buch, Profil und `bookconfig/`-Pfad leben ab dort; `BookStudio.load_book` schrumpft auf ≤ 5 Zeilen. **Status: erledigt (Commit `XXXXXXX`)**. |
