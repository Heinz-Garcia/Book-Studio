...
class AppConfigEditor(tk.Toplevel):
    ...
    def __init__(self, parent, config_path, on_save=None, templates=None):
        ...
        # B4: enable_footnote_backlinks & default_footnote_mode entfernt.
    ...
    def _build_ui(self):
        ...
        ttk.Separator(wrapper, orient="horizontal").grid(row=6, column=0, columnspan=3, sticky="we", pady=(6, 8))

        ttk.Label(wrapper, text="Default-Exportformat:").grid(row=7, column=0, sticky="w", pady=6)
        ttk.Combobox(
            wrapper,
        ...
        ttk.Label(wrapper, text="Default-Template:").grid(row=8, column=0, sticky="w", pady=6)
        ttk.Combobox(
            wrapper,
        ...
        ttk.Label(
            ...
            font=("Segoe UI", 8),
            foreground="#888888",
        ).grid(row=9, column=1, columnspan=2, sticky="w", pady=(0, 6))

        ttk.Label(wrapper, text="Log Auto-Clear Default:").grid(row=10, column=0, sticky="w", pady=6)
        ttk.Checkbutton(wrapper, variable=self.log_auto_clear_default_var).grid(row=10, column=1, sticky="w", pady=6)

        ttk.Label(wrapper, text="Log-Limit Default:").grid(row=11, column=0, sticky="w", pady=6)
        ttk.Combobox(
            wrapper,
        ...
        ttk.Separator(wrapper, orient="horizontal").grid(row=12, column=0, columnspan=3, sticky="we", pady=(8, 8))
        ttk.Label(wrapper, text="Tools: Merge-Quellpfade (; getrennt):").grid(row=13, column=0, sticky="w", pady=6)
        ttk.Entry(wrapper, textvariable=self.prep_sources_var, width=58).grid(row=13, column=1, sticky="we", pady=6)
        ttk.Button(wrapper, text="Ordner +", style="Tool.TButton", command=self._add_prep_source_folder).grid(row=13, column=2, sticky="e", padx=(8, 0), pady=6)

        ttk.Label(wrapper, text="Tools: Merge-Zielordner:").grid(row=14, column=0, sticky="w", pady=6)
        ttk.Entry(wrapper, textvariable=self.prep_dest_folder_var, width=58).grid(row=14, column=1, sticky="we", pady=6)
        ttk.Button(wrapper, text="Ordner...", style="Tool.TButton", command=self._browse_prep_dest_folder).grid(row=14, column=2, sticky="e", padx=(8, 0), pady=6)

        ttk.Label(wrapper, text="Tools: Indexer-Zielordner:").grid(row=15, column=0, sticky="w", pady=6)
        ttk.Entry(wrapper, textvariable=self.indexer_target_folder_var, width=58).grid(row=15, column=1, sticky="we", pady=6)
        ttk.Button(wrapper, text="Ordner...", style="Tool.TButton", command=self._browse_indexer_target_folder).grid(row=15, column=2, sticky="e", padx=(8, 0), pady=6)

        hint = (
            "Relative Pfade gelten relativ zum Book-Studio-Codeordner. "
            "Nach dem Speichern wird die Projektliste neu geladen."
        )
        ttk.Label(wrapper, text=hint, font=("Segoe UI", 8)).grid(row=16, column=0, columnspan=3, sticky="w", pady=(6, 10))

        button_row = ttk.Frame(wrapper)
        button_row.grid(row=17, column=0, columnspan=3, sticky="e", pady=(2, 0))
